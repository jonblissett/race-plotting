 function hh = quiverwcolorbar2_big(varargin)
...
% changed T. H�ffken 14.03.05, for high data resolution
...
if(nin == 4)  % quiver(u,v,s,vc)
    if(isscalar(varargin{nin}))
        scale = varargin{nin};
        vr = varargin{4};
    end
elseif(nin >= 5)  % quiver(x,y,u,v,s,vc) or quiver(x,y,u,v,s,'bounds',[start end])
...
colorbar('EastOutside','ytick',B,'yticklabel',C,...
    'ticklength',[0.04 0.1],'YLim',[B(1) B(ticknum)])
